...
<html>
<head>
<title>Mise à jour d'un niveau scolaire</title>
<meta charset="utf-8" />
...
<body class="is-preload no-sidebar">
			...
			<div class="container">
				<div id="content">
					...
					<%
						...
						String t2 = request.getParameter("SchoolLevel");
						...
						if ((t1 != null && t2 != null)
							...
							try {
								...
								DBManager.setLevel(idInt, t2);
								//Pour faire entrer un nouveau niveau
								t1 = null;
						...
						LEVEL myLevel = null;
						Integer freeLevelID = DBManager.getFreeLevelID();
						if (t1 == null) {
							idInt = freeLevelID;
							t1 = idInt.toString();
							myLevel = new LEVEL(idInt, "");
						} else {
							if (t2==null || t2.isEmpty() || myLevel==null) {
								idInt = Integer.parseInt(t1);
								myLevel = DBManager.getLEVEL_by_ID(idInt);
							} else {
								myLevel = new LEVEL(idInt, t2);
							}
						}

					%>
					<form id="levelFormId" name="levelForm" method="get"
						style="width: 50%; margin: auto; background-color: #c1d9fc; padding-bottom: 15px;" accept-charset="UTF-8">
						<p align="center">
						<h2>Informations du niveau scolaire</h2>
						</p>
						<%
						ArrayList<LEVEL> levels = DBManager.getLEVELS();
						%>
						<datalist id="idLevelList">
						<%
						for (int i = 0; i < levels.size(); i++) {
						%>
							<option><%=levels.get(i).ID%></option>
						<%
						}
						%>
						</datalist>
						...
						<table>
							<tr>
								...
								<th>
									<p>
										<input type="text" id="ID" name="ID" style="color: graytext" list="idLevelList" autocomplete="off" 
											value="<%=myLevel.ID%>" size=25 align="right" />
									</p>
								</th>
							</tr>
							<tr>
						...
						<p style="width: 100%; margin: auto;" align="center">
							...
							<INPUT type="button" id="SuppBtn" value="Supprimer un niveau" onclick="removeLevel()" />
						</p>
					</form>
					<div id="Result" style="color:red;"></div>
					<br />
					<%
						if (myLevel.ID < (freeLevelID-1)) {
					%>
					<a href="school_level.jsp?ID=<%=myLevel.ID + 1%>">Suivant</a><br />
					<%
						}
					%>
				</div>
	...
	$(document).ready(function() {
		$("#ID").change(function(event){
			var _idlevel = document.getElementById("ID").value;
	...
	function removeLevel() {
		...
		$.ajax({
			...
			success: function (result) {
				...
				document.getElementById("levelFormId").reset();
			},
